...
# SO + Complementary Shaders - Reimagined + Rethinking Voxels + Default Dark Mode + Enhanced Boss Bars + Even Better Enchants + Iris + Mod Menu + Sodium Extra + Reece's Sodium Options + LambDynamicLights + Continuity + Dynamic FPS + AppleSkin + Not Enough Animations + BetterF3 + 3D Skin Layers + Zoomify + Xaero's Minimap (Minimap: off, Deathpoints: off) + Borderless Mining + CIT Resewn + Inventory Profiles Next (Enable Items Highlight: false, Enable Overlay Editor Button: false, Enable Settings Button In Inventory: false, Last State Of Checkbox: false) + Presence Footsteps (volume: .3) + Xaero's World Map + Better Mount HUD + More Chat History + Shulker Box Tooltip + Mouse Tweaks + Eating Animation + Better Statistics Screen + Better Ping Display + Chat Heads + Boat Item View + EMI + ClearDespawn + Make Bubbles Pop + Enchantment Descriptions + Highlight + MacOS Input Fixes + Distant Horizons + Better Third Person + Detail Armor Bar + Status Effect Bars + Held Item Info + Chest Tracker
