...
# SO + Complementary Shaders - Reimagined + Rethinking Voxels + Default Dark Mode + Enhanced Boss Bars + Iris + Mod Menu + Sodium Extra + Reece's Sodium Options + LambDynamicLights + Continuity + AppleSkin + Not Enough Animations + BetterF3 + 3D Skin Layers + Zoomify + Xaero's Minimap + Inventory Profiles Next + Presence Footsteps (volume: .3) + Xaero's World Map + Roughly Enough Items + Better Mount HUD + More Chat History + Shulker Box Tooltip + Mouse Tweaks + Eating Animation + Ambient Sounds (volume: .3) + Better Statistics Screen + Better Ping Display + Chat Heads + Bobby + Boat Item View + ClearDespawn + Make Bubbles Pop + Enchantment Descriptions + Highlight + Gamma Utils + MacOS Input Fixes + Better Third Person + Detail Armor Bar + Status Effect Bars + Held Item Info
